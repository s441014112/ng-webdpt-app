...
export class SingleColumnComponent implements OnInit {
  ...
  /**
   * 获取单列组件的属性。
   * 这里进行类型断言，确保 props 具有 SingleColumnProps 接口的类型。
   */
  get singleColumnProps(): SingleColumnProps {
    return this.node.props as SingleColumnProps;
  }

  /**
   * 过滤出所有类型为 SLOT 的子组件。
   * 单列组件的子元素是插槽，插槽的子元素才是实际的组件。
   */
